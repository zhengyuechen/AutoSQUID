...
* **Segment-yield goal (`Config.segment_goal`).** A second, mutually-exclusive acquisition goal alongside `n_trials`: `segment_goal=(n_segments, segment_length)` keeps acquiring full-length `n_points` runs and stops once the **cumulative count of full clean `segment_length` windows** harvested from their clean pre-jump prefixes reaches `n_segments`. Motivation: at short scan intervals the lock slips before a full `n_points` run completes, but each partial run's clean prefix still holds several analysis windows (e.g. the eight Jun-05 500 µs JUMP traces hold 55 usable 1M-segments). **The DAQ run length is always `cfg.n_points` — `segment_goal` never changes acquisition length;** `segment_length` is the analysis-window size and the stopping unit only. `n_trials` and `segment_goal` are mutually exclusive (set exactly one). `min_usable_frac` gates trials mode only; in segment mode every CLEAN-prefix acquisition contributes `usable_points // segment_length` windows (a JUMP-truncated-but-clean prefix contributes; `SURGE`/`DEAD`/`BAD_BASELINE` contribute 0).
* **`goals.py`** — the goal/progress policy module (pure bookkeeping, no hardware), so `measurement.py` stays the hardware state machine: `goal_mode`, `validate_goal`, `target_count`, `unit`, `contribution`, `scan_progress`, `is_done`, `progress_str`. `run_cycle` calls these; its loop counter is the neutral `progress` (no longer `n_clean`).
* **`analysis.segment_progress` / `analysis.segment_trace_names`** — segment-mode resume count and the on-disk segment-mode trace filenames for a (interval, temp), matched on **exact `segment_length`** and `goal_mode=='segment'`. `segment_trace_names` is the segment-mode analogue of `accepted_trace_names` (segment-mode rows set `accepted=0`, so the trials-mode selector finds nothing). Phase-2 segment plotting will build on it.
* **`analysis.backfill_ledger(outdir, cfg=None, segment_length=None, dry_run=True)`** — post-run reconstruction of MISSING ledger values, distinct from the schema-only `_migrate_ledger`. Fills only blank fields it can confidently infer — physics/data (`usable_points`, `usable_seconds`, `scan_interval_us`, `mean_V`, `std_V`, `outcome`) from the saved DAQ `.txt` (read only when one is blank); `T_start_K`/`T_end_K` only from a matching `TEMP_*.csv`; goal fields (`goal_mode`, `segment_length`, `accepted`, `accepted_segments`, `goal_progress`, trials-mode `n_clean`) only from `cfg`/`segment_length` context; `attempt` from the filename index — and **never** overwrites a present value, invents `timestamp`/`n_resets`/`event`, or writes `nan`. `dry_run=True` (default) returns the proposed per-row fills without writing, e.g. to retro-fit `accepted_segments`/`goal_progress` onto an old trials-era ledger for a new `segment_goal`.
* **`plotting.plot_segment_psd_folder(path, scan_interval_us, segment_length, conversion=1, temp_label=None, locate_usable=True, clean_only=False, segment_clean_only=False, jump_v=0.5, rail_v=9.5, baseline_chunks=1, …)`** — LEDGER-FREE per-segment PSDs. Scans a folder for one scan interval's DAQ files (`DAQ_*_{tau}us_*pts_*.txt`, excluding `_SURGE`/`_BADBASE`; optional `temp_label` filter). With **`locate_usable=True` (default)** it runs `truncate_to_clean` per file and segments only the clean pre-jump prefix — a new AutoSQUID file (already a clean prefix) comes back whole, a legacy file saved WITH its post-jump tail is cut before segmentation; `False` segments each file exactly as saved (debugging). It splits the located data into full `segment_length` blocks, takes one one-sided Welch PSD per block, and overlays them — each segment faint gray, the **mean** across blocks in black. `clean_only` gates the WHOLE located prefix with `is_surge_spec` (skips the file); `segment_clean_only` instead gates each individual `segment_length` window (a failing window is dropped from the PSD and from `meta`). Returns `(f, psd_stack, meta)` — `meta` has one row per kept segment (`filename`, `segment_index`, `start_point`, `end_point`, `scan_interval_us`, `temp_label`, `n_points_file`, `usable_points`, `usable_seconds`, `locator_kind`, `locator_reason`, `segment_ok`, `segment_reason`), so you can tell whether a file was used whole or truncated. Works on folders with no ledger; note adjacent blocks of one acquisition are not independent acquisitions.

### Changed

* **Experiment ledger columns:** added `goal_progress` (active-unit cumulative — traces or segments), `goal_mode`, `segment_length`, and `accepted_segments` (this row's segment contribution; numeric, never blank). `n_clean` stays **trace-count-only** (blank in segment mode) and `accepted` keeps its trace-acceptance meaning (`0` in segment mode) — neither is overloaded to mean segments. Resume in segment mode sums `accepted_segments`; trials mode is unchanged. (One by-name ledger migration; old folders upgrade in place.)
* **Notebook §0 / builder:** the target block now offers the two mutually-exclusive goals (Goal A `n_trials` + `min_usable_frac`; Goal B `segment_goal`), and the header line prints the active goal via `target_count` / `goal_unit`.

### Fixed

* **`segment_goal` is now strictly validated** (`goals.validate_goal`): rejects both-set / neither-set, a non-positive or non-integer `n_trials`, a malformed `segment_goal` (not a positive-integer 2-tuple, e.g. `(0, 1_000_000)` / `(20, 0)` / negatives), and `n_points < segment_length`.
